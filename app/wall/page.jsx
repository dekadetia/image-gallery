...
function parseImageMeta(dimensions) {
  ...
  return {
    ...
    /*
      Use actual pixel dimensions whenever available.

      So:
      "1.33:1 | 1436×1080"
      renders at 1436 / 1080 = 1.329629...
      rather than being forced to exactly 1.33.
    */
...
/* ---------------------------------------------------------
   ...
   Each number represents the number of images vertically
...
/* ---------------------------------------------------------
   ...
   Every column must wind up at exactly the same height.

   For a column:

   image height = image width / aspect ratio

   If there are multiple stacked images:
   ...
   We solve for the shared band height, while also subtracting
   the 10px horizontal gutters from available width.
--------------------------------------------------------- */
...
function buildBand(images, pattern, containerWidth) {
  ...
  const columns = pattern.map(count => {
    const items = images.slice(cursor, cursor + count)

    cursor += count

    /*
      If the column width is W:

      image height = W / ratio

      Therefore total image-only column height is:
  ...
  /*
    We need every column to have the same final height H.
    ...
    And all column widths combined must equal
    availableImageWidth.

    Solve H algebraically.
  */

  const denominator = columns.reduce(
    (sum, column) => {
      return sum + 1 / column.stackWeight
    },
    0
  )

  const gapAdjustment = columns.reduce(
    (sum, column) => {
      return (
        sum +
  ...
  const bandHeight =
    (availableImageWidth + gapAdjustment) /
    denominator


  const solvedColumns = columns.map(column => {
    const width =
      (bandHeight - column.verticalGapHeight) /
      column.stackWeight
  ...
  /*
    Reject pathological solutions.

    This would only really happen if the viewport became
    extraordinarily narrow relative to the chosen pattern.
  */
...
/* ---------------------------------------------------------
   ...
   Completed bands remain deterministic.

   If InfiniteScroll gives us an incomplete last band, we
   simply don't render those final few images yet.

   When more arrive, they complete that band.

   Everything above stays exactly where it was.
--------------------------------------------------------- */

function buildWall(images, containerWidth) {
  if (!containerWidth || !images.length) {
    return []
  }

  const preparedImages = images.map(image => ({
    ...image,
    _meta: parseImageMeta(image.dimensions)
  }))

  const patterns = getPatterns(containerWidth)
  ...
  while (imageCursor < preparedImages.length) {
    const pattern =
      patterns[bandIndex % patterns.length]

    const requiredImages = pattern.reduce(
      (sum, count) => sum + count,
      0
    )

    const remaining =
      preparedImages.length - imageCursor

    if (remaining < requiredImages) {
      break
    }

    const bandImages = preparedImages.slice(
      imageCursor,
      imageCursor + requiredImages
    )
...
function TetrisWall({
  ...
  const bands = useMemo(
    () => buildWall(images, containerWidth),
      ...
      {bands.map((band, bandIndex) => (
        ...
        >
          {band.columns.map(
            (column, columnIndex) => (
              <div
                key={`column-${bandIndex}-${columnIndex}`}
...
export default function Tetris() {
  ...
  const slides = Images.map(photo => {
    ...
    const meta =
      parseImageMeta(photo.dimensions)
