...
export default function Scrl() {
  ...
  const getImages = async load => {
    if (load !== 'load more') __loader(true)
    ...
    try {
      ...
      if (response.ok) {
        ...
        const uniqueImages = images.filter(img => !seenImageIds.current.has(img.id))
  ...
  const handleUserActivity = () => {
    ...
    activityTimerRef.current = setTimeout(() => setShowControls(false), 5000)
  }
