...
export default function Order() {
  ...
  // 🧠 Keep track of last Fuse query and result subset
  ...
  const fuse = useMemo(() => new Fuse(FullImages, fuseOptions), [FullImages])
  ...
  const getImages = async (token) => {
    try {
      const res = await fetch(`/firebase/get-ordered-images`, {
        method: 'POST',
  ...
  const getAllImagesNoLimit = async () => {
    try {
      const res = await fetch(`/firebase/get-all-images-no-limit`)
      const data = await res.json()
  ...
  // 🔹 Debounced incremental Fuse search
  useEffect(() => {
    if (debounceRef.current) clearTimeout(debounceRef.current)
    debounceRef.current = setTimeout(async () => {
      const raw = searchQuery.trim().toLowerCase()

      // ⛔ if cleared, reset to default ordered grid
      if (!raw) {
      ...
      // Abort any pending backend search
      if (searchAbortRef.current) searchAbortRef.current.abort()

      const prev = lastQueryRef.current
      const isPrefix = raw.startsWith(prev)
      let base = FullImages

      // ✅ Narrow subset if prefix search
      if (isPrefix && lastResultsRef.current.length) {
        base = lastResultsRef.current
      }

      const fuseInstance = new Fuse(base, fuseOptions)
      const fuseResults = fuseInstance.search(raw, { limit: 200 }).map(r => r.item)

      // ✅ fallback to backend if Fuse empty
      if (fuseResults.length === 0) {
        const controller = new AbortController()
        searchAbortRef.current = controller
        try {
          const res = await fetch(`/firebase/search-ordered-images`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          ...
          if (err.name !== 'AbortError') console.error('Backend search error:', err)
        }
        return
      }
      ...
      lastResultsRef.current = fuseResults
