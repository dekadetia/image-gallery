...
/* ---------------------------------------------------------
   ...
   Raw asset:
     /images/39767.the.pilgrim.1923.webp

   Lightbox permalink:
     /view/39767.the.pilgrim.1923.webp
--------------------------------------------------------- */

function getImageSlug(image) {
  const name =
    image?.name ||
    image?.id ||
    ''
...
function matchesImagePermalink(
  image,
  requested
) {
  if (!requested) {
  ...
  return (
    image?.id === decoded ||
    image?.name === decoded ||
    getImageSlug(image) === decoded
  )
}


function getRequestedImageFromLocation() {
  const pathname =
  ...
  // Temporary backwards compatibility with the query-string
  // deep links generated by the earlier build.
  const params =
    new URLSearchParams(
...
function getPermalinkPath(image) {
  const filename =
    image?.name ||
    image?.id ||
    ''
...
export default function Page() {
  ...
  const [deepLinkImage, setDeepLinkImage] = useState(null)
